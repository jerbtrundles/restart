...
def apply_spell_effect(caster: CasterTargetType, target: CasterTargetType, spell: Spell, viewer: Optional[ViewerType]) -> Tuple[int, str]:
    ...
    if spell.effect_type in ["unlock", "lock"]:
        if isinstance(target, Container):
            ...
            return 0, f"The {spell.name} spell has no effect on {getattr(target, 'name', 'target')}."

    # --- 2. Standard Combat/Healing Logic ---
    # (Existing logic for damage calculation, etc.)
    ...
    if spell.effect_type == "damage" and spell.damage_type != "physical" and hasattr(target, 'get_resistance'):
        ...
        if flavor_key:
            ...
            if message_template:
                formatted_target_for_flavor = f"{FORMAT_HIGHLIGHT}{target.name}{FORMAT_RESET}"
    ...
    if spell.effect_type == "damage":
        ...
        summon_level = 10 # Logic simplification for now
    ...
    if not message:
         ...
         except KeyError: message = f"{formatted_caster_name} used {spell.name} on {formatted_target_name}."
