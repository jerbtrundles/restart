...
Explicitly manages TextFormatter state and captures hotspots for clickable text.
"""
import time
import pygame
    ...
    try:
        ...
        if friendly_targets:
            for target in friendly_targets:
                ...
                # --- FIX: CAPTURE HOTSPOTS IMMEDIATELY ---
...
def draw_room_info_panel(renderer: 'Renderer', world: 'World'):
    ...
    panel_usable_width = max(1, panel_rect.width - padding * 2)
    ...
    try:
        ...
        # Capture for the single big render block
        renderer._static_hotspots.extend(renderer.text_formatter.last_hotspots)
    finally:
        renderer.text_formatter.update_screen_width(original_width)