...
class GameManager:
    ...
    def handle_input(self, text: str) -> str:
        ...
        if not self.world.player.is_alive:
             return f"{TextFormatter.FORMAT_ERROR}You are dead. You cannot do that.{TextFormatter.FORMAT_RESET}"
    ...
    def update(self):
        ...
        if not self.world.player.is_alive and self.game_state == "playing":
             ...
             self.text_buffer.append(f"{TextFormatter.FORMAT_ERROR}{TextFormatter.FORMAT_TITLE}YOU HAVE DIED{TextFormatter.FORMAT_RESET}")
    ...
    def draw(self):
        ...
        if self.game_state == "game_over":
            ...
            death_surface = game_over_font.render(death_text, True, config.FORMAT_COLORS[config.FORMAT_ERROR])
            respawn_surface = self.font.render(respawn_text, True, config.TEXT_COLOR)
    ...
    def resize_screen(self, new_width, new_height):
        # ... (implementation unchanged) ...
        config.SCREEN_WIDTH = new_width; config.SCREEN_HEIGHT = new_height
    ...
    def handle_respawn(self):
        ...
        # Add respawn message
        self.text_buffer = [f"{TextFormatter.FORMAT_HIGHLIGHT}You feel your spirit return to your body...{TextFormatter.FORMAT_RESET}\n"]
                    ...
                    if event.type == pygame.KEYDOWN:
                     ...
                     if event.type == pygame.KEYDOWN:
                          if event.key == pygame.K_RETURN:
                               ...
                               if self.debug_mode: self.text_buffer.append(f"{TextFormatter.FORMAT_HIGHLIGHT}Debug mode enabled. Press F1 to disable.{TextFormatter.FORMAT_RESET}")
                               else: self.text_buffer.append(f"{TextFormatter.FORMAT_HIGHLIGHT}Debug mode disabled.{TextFormatter.FORMAT_RESET}")
                          else:
                               if event.unicode.isprintable(): self.input_text += event.unicode
                     elif event.type == pygame.MOUSEWHEEL: # ... (scrolling - unchanged) ...
                          if event.y > 0: self.scroll_offset = min(len(self.text_buffer) * 20, self.scroll_offset + config.SCROLL_SPEED)
                          elif event.y < 0: self.scroll_offset = max(0, self.scroll_offset - config.SCROLL_SPEED)
    ...
    def _draw_status_indicators(self):
        # ... (implementation unchanged) ...
        if not hasattr(self.world, 'player'): return
        player = self.world.player
        health_width = 100; health_height = 5; health_x = 10; health_y = self.layout["status_area"]["y"] + 8
        ...
        pygame.draw.rect(self.screen, health_color, (health_x, health_y, filled_width, health_height))
        health_text = f"HP: {player.health}/{player.max_health}"; health_surface = self.font.render(health_text, True, (255, 255, 255))
        self.screen.blit(health_surface, (health_x + health_width + 10, health_y - 5))
        if hasattr(player, 'experience') and hasattr(player, 'experience_to_level'):
            ...
            self.screen.blit(xp_surface, (self.layout["screen_width"] - xp_surface.get_width() - 10, health_y - 5))
        pygame.draw.line(self.screen, (80, 80, 100), (0, self.layout["status_area"]["y"] - 1), (self.layout["screen_width"], self.layout["status_area"]["y"] - 1), 1)
    ...
    def _calculate_layout(self):
        # ... (implementation unchanged) ...
        self.layout = {"screen_width": config.SCREEN_WIDTH, "screen_height": config.SCREEN_HEIGHT,
                       "time_bar": {"height": 30, "y": 0},
                       "status_area": {"height": 20, "y": config.SCREEN_HEIGHT - 20},
                       "input_area": {"height": config.INPUT_HEIGHT, "y": config.SCREEN_HEIGHT - config.INPUT_HEIGHT - 20}}
        self.layout["text_area"] = {"y": self.layout["time_bar"]["height"] + 10,
                                   "height": (self.layout["input_area"]["y"] - (self.layout["time_bar"]["height"] + 20))}
        line_height = self.font.get_linesize() + config.LINE_SPACING
        self.max_visible_lines = self.layout["text_area"]["height"] // line_height if line_height > 0 else 20 # Prevent division by zero
